# -*- coding: utf-8 -*-
"""P1-1 小红书长文配图（2张竖版数据图，1080×1440 · 3:4）
长文文字复用今日头条长文，配图为轻量辅助。1. 四句话数据速览 2. 给家长三句话。
"""
import numpy as np
...
BASE = "E:/1.HSEE/6.ContentCrawlKits/docs/03-内容创作-深圳中考志愿填报/P1-政策解码器/P1-1-游戏规则全景/04.小红书/03.长文/"
...
put(d, ck, "深圳中考游戏规则 · 关键数字速览", font(34, True), (540, 100), color=GOLD)
put(d, ck, "四句话，一条看懂", font(56, True), (540, 215), color=WHITE, maxw=1000)
stats = [
    ("630", "总分 · 8科 · 语数英物化占70%"),
    ("440/170", "主战场440 · 定公办民办170"),
    ("23%", "D类公办指标 · 占考生一半以上"),
    ("16", "第一批志愿 · 分高先挑顺序你定"),
]
...
for (num, lab), (bx, by) in zip(stats, pos):
    ...
    put(d, ck, num, font(62, True), (bx + cw // 2, by + 78), color=GOLD, maxw=380)
    put(d, ck, lab, font(26), (bx + cw // 2, by + 168), color=WHITE, maxw=380)
d.line([540 - 110, 950, 540 + 110, 950], fill=GOLD, width=4)
put(d, ck, "录取分五批次 · 前一批录了后面全作废", font(34, True), (540, 1050), color=GOLD, maxw=1000)
put(d, ck, "搞懂规则 · 填志愿不踩坑 · 不犯低级错误就是优势", font(30), (540, 1136), color=LIGHT, maxw=1000)
put(d, ck, "数据来源：《2026年深圳市高中阶段学校考生报考指导手册》· 逐条人工核对", font(24), (540, 1350), color=SUB)
results["data"] = verify(d, ck, "1 数据速览")
save(img, N + "-长文1-四句话速览-1080x1440.png")
...
put(d, ck, "孩子负责学习好 · 家长负责决策优 · 出分前做功课", font(34, True), (540, 1180), color=GOLD, maxw=1020)
