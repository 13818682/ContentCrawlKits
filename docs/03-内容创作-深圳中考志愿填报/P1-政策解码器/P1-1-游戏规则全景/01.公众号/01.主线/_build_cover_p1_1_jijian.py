# -*- coding: utf-8 -*-
"""
P1-1 主线 · 公众号首图 · 极简大字版（900×383）
========================================
要素与精简版首图一致（徽章/「深圳中考游戏规则/30分钟从入门到看懂」/副题/CTA胶囊），
但排版改为「630分」巨型数字对撞大字版——关键数字放大为视觉主角，与精简版两行46px居中区分。
...
put(d, ck, "深圳中考 · P1系列 · 游戏规则全景", font(15, True), (450, 39), color=GOLD)

# ---------- 要素2 · 主标题（精简版两行居中 → 极简版改为单行） ----------
put(d, ck, "深圳中考游戏规则", font(32, True), (450, 84), color=WHITE, maxw=820)

# ---------- 要素3 · 「630」巨型数字对撞（视觉主角） ----------
put(d, ck, "总分630 · 8科", font(19, True), (285, 132), color=LIGHT)
put(d, ck, "五批次 · 16志愿", font(19, True), (635, 132), color=LIGHT)
put(d, ck, "630", font(96, True), (285, 208), color=GOLD, maxw=360)
put(d, ck, "16", font(72, True), (635, 212), color=GOLD, maxw=390)
d.rounded_rectangle([398, 182, 502, 236], radius=25, fill=GOLD)
put(d, ck, "分", font(30, True), (450, 209), color=NAVY)

# ---------- 要素4 · 副题（同精简版） ----------
put(d, ck, "语数英物化440分占70% · 每1分都有它的位置", font(20, True), (450, 290), color=GOLD, maxw=820)
...
if not (px0 >= 10 and px1 <= W - 10 and py0 >= 10 and py1 <= H - 20):
    bad += 1
    print(f"[胶囊越界/贴边] rect=({px0:.0f},{py0:.0f},{px1:.0f},{py1:.0f}) 底部留白={H - py1:.0f}px")
print(f"OVERFLOW: {'PASS' if bad == 0 else 'FAIL ' + str(bad)}（胶囊底部留白 {H - py1:.0f}px）")

out = "E:/1.HSEE/6.ContentCrawlKits/docs/03-内容创作-深圳中考志愿填报/P1-政策解码器/P1-1-游戏规则全景/01.公众号/01.主线/P1-1-主线-公众号-首图-极简大字版-900x383.png"
img.save(out)
