# -*- coding: utf-8 -*-
"""S5 公众号封面 精简版（900×383，3行，对齐 S6-B 风格）
深圳中考保留 + 第二句主题保留（A+永远是前5%）+ 第三/四句整合为 1 句钩子：
  原第三句「别让任何一科掉 C」× 原第四句「省一级公办普高的隐形门槛」
  → 整合「一科掉 C，就够不上省一级」（后果悬念，吸引点击）。
封面第一眼=深圳中考+问题；数字≤2（仅 5%）。
"""
...
# ---------- S5 公众号封面 精简版（3行） ----------
im, d = base()
put(d, "深圳中考", (W / 2, 52), 40, GOLD, tag="s5b")
f = put(d, "A+ 永远是前 5%", (W / 2, 170), 56, WHITE, maxw=860, min_size=44, tag="s5b")
bb = d.textbbox((W / 2, 170), "A+ 永远是前 5%", font=f, anchor="mm")
half = (bb[2] - bb[0]) / 2 + 4
d.line([W / 2 - half, 232, W / 2 + half, 232], fill=GOLD, width=5)
put(d, "一科掉 C，就够不上省一级", (W / 2, 312), 44, GOLD, maxw=880, min_size=36, tag="s5b")
im.save(HERE + "S5-单科等级与省一级门槛-公众号-封面-精炼版-900x383.png")
check("S5 公众号封面 精简版")
print("done")
