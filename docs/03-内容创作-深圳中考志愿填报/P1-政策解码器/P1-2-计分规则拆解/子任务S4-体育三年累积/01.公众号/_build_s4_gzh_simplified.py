# -*- coding: utf-8 -*-
"""S4 公众号封面 精简版（900×383，3行，对齐 S5/S6 精简版风格）
深圳中考保留 + 第二句主题保留（体育50分·一半靠攒）+ 第三/四句整合为 1 句钩子：
  原第三句「14分从初一攒起·别等4月」为主；原第四句「过程分+现场考=50分」与主标题重复，舍弃。
  → 整合「14 分从初一就攒 · 别等考前突击」（时间紧迫钩子，吸引点击）。
封面第一眼=深圳中考+问题；数字≤2（50、14）。
"""
...
# ---------- S4 公众号封面 精简版（3行） ----------
im, d = base()
put(d, "深圳中考", (W / 2, 52), 40, GOLD, tag="s4b")
f = put(d, "体育 50 分，一半靠攒", (W / 2, 170), 56, WHITE, maxw=860, min_size=44, tag="s4b")
bb = d.textbbox((W / 2, 170), "体育 50 分，一半靠攒", font=f, anchor="mm")
half = (bb[2] - bb[0]) / 2 + 4
d.line([W / 2 - half, 232, W / 2 + half, 232], fill=GOLD, width=5)
put(d, "14 分从初一就攒 · 别等考前突击", (W / 2, 312), 44, GOLD, maxw=880, min_size=36, tag="s4b")
im.save(HERE + "S4-体育三年累积-公众号-封面-精炼版-900x383.png")
check("S4 公众号封面 精简版")
print("done")
