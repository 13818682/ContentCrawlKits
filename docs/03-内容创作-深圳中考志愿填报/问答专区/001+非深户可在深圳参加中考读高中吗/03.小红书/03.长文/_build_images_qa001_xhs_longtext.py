# -*- coding: utf-8 -*-
"""QA-001 小红书长文配图（2张竖版数据图，1080×1440 · 3:4）
长文文字复用今日头条长文，配图为轻量辅助（1-2张数据图即可）。
1. 数据速览：54%/23%/9,186/73% 四个关键数字
2. 三句话：给非深户家长的行动三句话
...
BASE = "E:/1.HSEE/6.ContentCrawlKits/docs/03-内容创作-深圳中考志愿填报/问答专区/001+非深户可在深圳参加中考读高中吗/03.小红书/03.长文/"
N = "01-QA-001-非深户能在深圳读高中吗-小红书-长文"
...
put(d, ck, "非深户 · 关键数字速览", font(34, True), (540, 100), color=GOLD)
put(d, ck, "一条看懂竞争与出路", font(56, True), (540, 215), color=WHITE, maxw=1000)
stats = [
    ("54%", "D类考生占比"),
    ("23%", "公办普高D类指标"),
    ("9,186", "D类指标生名额"),
...
for (num, lab), (bx, by) in zip(stats, pos):
    ...
    put(d, ck, num, font(96, True), (bx + cw // 2, by + 92), color=GOLD, maxw=370)
    put(d, ck, lab, font(32), (bx + cw // 2, by + 190), color=WHITE, maxw=360)
d.line([540 - 110, 950, 540 + 110, 950], fill=GOLD, width=4)
put(d, ck, "四大名校 AC线 vs D线 只差 0-5分", font(38, True), (540, 1052), color=GOLD, maxw=1000)
put(d, ck, "分数越高，户籍差距越小", font(30), (540, 1136), color=LIGHT)
put(d, ck, "数据来源：深圳市教育局公开信息 · 逐条人工核对", font(24), (540, 1350), color=SUB)
results["data"] = verify(d, ck, "1 数据速览")
save(img, N + "-长文1-数据速览-1080x1440.png")

# ================= 2. 给家长三句话 =================
img, d, ck = new_canvas(1)
put(d, ck, "给非深户家长 · 三句话", font(34, True), (540, 100), color=GOLD)
put(d, ck, "记住这3句，心里不慌", font(56, True), (540, 215), color=WHITE, maxw=1000)
lines = [
    ("先过资格关", "5项材料现在核对，别等3月报名"),
    ("用D线定位", "别拿AC线吓自己"),
    ("三条腿走路", "公办+民办+中职，别只盯公办一条路"),
]
y0, rh, rg = 340, 200, 40
...
put(d, ck, "收藏 · 转给还在焦虑的非深户家长", font(36, True), (540, 1180), color=GOLD)
put(d, ck, "详细数据看上一张 →", font(26), (540, 1360), color=SUB)
