# -*- coding: utf-8 -*-
"""QA-002 小红书长文配图（2张竖版数据图，1080×1440 · 3:4）
...
BASE = "E:/1.HSEE/6.ContentCrawlKits/docs/03-内容创作-深圳中考志愿填报/问答专区/002+深圳中考：普高和中职每年学费差多少？/03.小红书/03.长文/"
...
put(d, ck, "普高 vs 中职 · 关键数字速览", font(34, True), (540, 100), color=GOLD)
put(d, ck, "一条看懂学费差", font(56, True), (540, 215), color=WHITE, maxw=1000)
stats = [
    ("约一半", "考生进不了公办普高"),
    ("21万-36万", "民办普高三年"),
    ("25-40倍", "一年比公办贵"),
...
for (num, lab), (bx, by) in zip(stats, pos):
    ...
    put(d, ck, num, font(84, True), (bx + cw // 2, by + 92), color=GOLD, maxw=370)
    put(d, ck, lab, font(32), (bx + cw // 2, by + 190), color=WHITE, maxw=360)
d.line([540 - 110, 950, 540 + 110, 950], fill=GOLD, width=4)
put(d, ck, "公办中职免学费 · 里面还有3+4贯通本科的路", font(36, True), (540, 1052), color=GOLD, maxw=1000)
put(d, ck, "便宜是真便宜，难进也是真难进", font(30), (540, 1136), color=LIGHT)
...
lines = [
    ("先算清账，再填志愿", "学费不是以后的事，是出分当天就要面对的事"),
    ("先定预算，再谈志愿", "家里三年能扛多少，决定志愿表下限"),
    ("三条腿走路", "公办优先 · 民办比价 · 中职看通道"),
]
y0, rh, rg = 340, 200, 40
...
put(d, ck, "学费这件事，永远是越早知道越好 · 收藏转给家长", font(34, True), (540, 1180), color=GOLD, maxw=1020)
put(d, ck, "详细数字看上一张 →", font(26), (540, 1360), color=SUB)
