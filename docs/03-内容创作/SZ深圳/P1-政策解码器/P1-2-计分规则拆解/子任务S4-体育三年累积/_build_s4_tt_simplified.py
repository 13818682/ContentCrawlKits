# -*- coding: utf-8 -*-
"""S4 今日头条封面 精简版（1200×900，对齐公众号精简理念）
深圳中考 pill 保留 + 主题主标题保留放大 + 一句金色钩子 + 来源。
钩子整合原「14分从初一攒·36分现场考」与「别等4月才突击」→「一半靠攒：14分从初一就在攒」。
数字≤2（50、14）。
"""
...
f = put(d, "体育 50 分，一半靠攒", (W / 2, 360), 118, WHITE, maxw=1100, min_size=90, tag="s4tt")
bb = d.textbbox((W / 2, 360), "体育 50 分，一半靠攒", font=f, anchor="mm")
half = (bb[2] - bb[0]) / 2 + 6
d.line([W / 2 - half, 480, W / 2 + half, 480], fill=GOLD, width=6)
put(d, "攒的 14 分，从初一就开始", (W / 2, 620), 68, GOLD, maxw=1080, min_size=54, tag="s4tt")
put(d, "数据来源：深圳市教育局公开信息 · 人工核对", (W / 2, 858), 24, SUB, bold=False, min_size=18, tag="s4tt")
im.save(HERE + "02.今日头条/S4-体育三年累积-头条-封面-精炼版-1200x900.png")
check("S4 头条封面 精简版")
print("done")
