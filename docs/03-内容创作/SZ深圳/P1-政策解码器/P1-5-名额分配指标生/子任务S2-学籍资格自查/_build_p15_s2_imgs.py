...
if __name__ == "__main__":
    import sys as _sys
    if "dy7" in _sys.argv:
        # ---- 只补渲抖音第 7 帧（2026-09-21 新增·主页引导帧）----
        # 起因：09-21 周报实测抖音「观看→主页」率 0.34%（小红书 3.4% 的 1/10），
        #      视频内当时无任何进主页引导。原 6 镜已定稿不回溯，故**加尾帧**而非改前 6 镜。
        # 用法：python _build_p15_s2_imgs.py dy7   （不加参数=原全量，行为不变）
        dy_card(7, "完整版在主页", "点我头像", "一整套自查清单在主页",
                [("① 完整自查清单 · 已在主页", 0), ("② 这一系列还有 3 篇", 0)],
                "主页里按顺序看", bigsize=112, bigmini=88)
        _sys.exit(0)
