...
def brand(d, text):
    """首图品牌行（2026-09-12 用户要求）：「深圳中考」大号金字，手机上一眼可辨。"""
    d.text((X0, 48), "深圳中考", font=font(52), fill=(255,210,120), anchor="la")
    lab = text.split("·", 1)[1].strip() if "·" in text else text
    d.text((W - X0, 65), lab, font=font(26), fill=(190,215,250), anchor="ra")
...
COVERS = {
  ...
  "子任务S2-同分PK": [
    ...
    ("P1-4-S2-同分PK-头条-封面3-门槛-1200x900.png",
     ["没成绩 →", "不能投档"], "官方文件写明：没有此项成绩，录取时不能投档"),
  ],
  "子任务S3-四个投档错误自查": [
    ("P1-4-S3-四错自查-头条-封面1-主标题-1200x900.png",
     ["投档 4 个坑，", "第 2 个最贵"], "一失手，12 个志愿全灭"),
    ("P1-4-S3-四错自查-头条-封面2-案例-1200x900.png",
     ["考 560 浪费分", "考 520 直接全灭"], "不是分低，是志愿没有容错"),
    ("P1-4-S3-四错自查-头条-封面3-分档表-1200x900.png",
     ["95 所公办普高", "铺满 487→592 分"], "光 520-549 这 30 分 · 就挤了 22 所"),
  ],
  "子任务S4-走读调剂与保存确认": [
    ("P1-4-S4-走读调剂保存-头条-封面1-主标题-1200x900.png",
     ["走读调剂该不该勾？", "89 所走读线平均低 11 分"], "分数够得着 ＋ 通勤能接受，才勾"),
    ("P1-4-S4-走读调剂保存-头条-封面2-89所差值-1200x900.png",
     ["89 所，全部更低", "一所例外都没有"], "最狠的：深实验崇文 住宿506 / 走读471（低35）"),
    ("P1-4-S4-走读调剂保存-头条-封面3-保存确认-1200x900.png",
     ["保存 ≠ 确认", "别白填"], "最多 3 次确认 · 截止 6月1日 18:00"),
  ],
}
